...
export function TodoList() {
  ...
  // Separate todos into in-progress and completed today
  const inProgressTodos = todos.filter(t => !t.is_completed);
  ...
  const handleAddTodo = async (e: React.FormEvent) => {
    ...
    try {
      await createTodo({
        text: newTodoText,
        order_index: inProgressTodos.length,
      });
      setNewTodoText('');
    } catch (error) {
  ...
  const handleClearInProgress = async () => {
    if (inProgressTodos.length === 0) return;
    if (confirm(`Clear all ${inProgressTodos.length} in-progress todos?`)) {
      try {
  ...
  const handleDrop = async (e: React.DragEvent, targetTodoId: number) => {
    e.preventDefault();
    if (!draggedId || draggedId === targetTodoId) return;

    // Reorder only within in-progress todos
    const draggedIndex = inProgressTodos.findIndex(t => t.id === draggedId);
    const targetIndex = inProgressTodos.findIndex(t => t.id === targetTodoId);

    if (draggedIndex === -1 || targetIndex === -1) return;

    // Create new order
    const reordered = [...inProgressTodos];
